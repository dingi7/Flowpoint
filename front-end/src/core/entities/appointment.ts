...
export const appointmentDataSchema = appointmentDataBaseSchema.refine(
  (data) => {
  ...
  {
    message: "Cannot cancel appointments that are more than 24 hours in the past",
    path: ["status"],
  }
);
...
export const appointmentSchema = baseEntitySchema.merge(appointmentDataBaseSchema).refine(
  (data) => {
    // Validate startTime is a valid ISO date string
    try {
      const startDate = new Date(data.startTime);
