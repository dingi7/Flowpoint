...
export function InvitationCard({ invitation, onAccept, onDecline }: InvitationCardProps) {
  ...
  const { data: organization, isLoading: isLoadingOrganization } = useGetOrganizationById(invitation.organizationId);
  
  // Calculate expiry date
  const expiryDate = addDays(new Date(invitation.createdAt), invitation.validFor || 7);
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="text-muted-foreground">Invited to organization:</span>
            <span className="font-medium text-xs bg-muted px-2 py-1 rounded">
              {isLoadingOrganization ? "Loading..." : organization?.name || invitation.organizationId}
            </span>
          </div>
