...
import { MoreHorizontal, Eye, Edit, Trash2, Clock, DollarSign, CheckCircle, X } from "lucide-react"
...
const mockCustomers = {
  "1": { id: "1", name: "Sarah Johnson", email: "sarah.johnson@email.com", phone: "+1 (555) 123-4567", avatar: "/abstract-geometric-shapes.png" },
  "2": { id: "2", name: "Mike Chen", email: "mike.chen@email.com", phone: "+1 (555) 234-5678", avatar: "/abstract-geometric-shapes.png" },
  "3": { id: "3", name: "Emma Wilson", email: "emma.wilson@email.com", phone: "+1 (555) 345-6789", avatar: "/abstract-geometric-shapes.png" },
  "4": { id: "4", name: "David Park", email: "david.park@email.com", phone: "+1 (555) 456-7890", avatar: "/abstract-geometric-shapes.png" },
  "5": { id: "5", name: "Lisa Zhang", email: "lisa.zhang@email.com", phone: "+1 (555) 567-8901", avatar: "/abstract-geometric-shapes.png" },
}
...
export function AppointmentList({ searchQuery, statusFilter, dateFilter }: AppointmentListProps) {
  ...
  const filteredAppointments = mockAppointments.filter((appointment) => {
    const customer = mockCustomers[appointment.customerId as keyof typeof mockCustomers]
    const service = mockServices[appointment.serviceId as keyof typeof mockServices]
    ...
    const appointmentDate = appointment.startTime ? new Date(appointment.startTime).toISOString().split("T")[0] : today
  ...
  return (
    <>
      <Card>
        <CardHeader>
          <CardTitle className="font-sans">Appointments ({filteredAppointments.length})</CardTitle>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
            ...
            <TableBody>
              {filteredAppointments.map((appointment) => {
                const customer = mockCustomers[appointment.customerId as keyof typeof mockCustomers]
                const service = mockServices[appointment.serviceId as keyof typeof mockServices]
                const appointmentDate = appointment.startTime ? new Date(appointment.startTime).toLocaleDateString() : 'N/A'
                const appointmentTime = appointment.startTime ? new Date(appointment.startTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}) : 'N/A'
                ...
                <TableRow key={appointment.id}>
                  ...
                  <TableCell>
                    <div>
                      <p className="font-medium">{service?.name || appointment.title}</p>
                      <p className="text-sm text-muted-foreground">
                        {appointment.description && appointment.description.length > 30
      ...
      <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
        <DialogContent className="min-w-3xl">
          ...
          {editingAppointment && (
            <AppointmentForm appointment={editingAppointment} onSuccess={() => setIsEditOpen(false)} />
          )}
        </DialogContent>
      </Dialog>
    </>
