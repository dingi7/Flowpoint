...
import { Organization, EmailTemplate } from "@/core";
import { useCurrentOrganizationId, useOrganizationActions } from "@/stores/organization-store";
...
const EMAIL_TEMPLATE_VARIABLES = [
  ...
  { name: "appointmentDate", label: "Appointment Date", example: "Monday, January 15, 2024 at 2:00 PM EET" },
  { name: "duration", label: "Duration", example: "1 hour" },
  { name: "fee", label: "Fee", example: "$50.00" },
  { name: "organizationName", label: "Organization Name", example: "My Business" },
  { name: "organizationAddress", label: "Organization Address", example: "123 Main St" },
  { name: "organizationPhone", label: "Organization Phone", example: "+1 234 567 8900" },
  { name: "organizationEmail", label: "Organization Email", example: "contact@example.com" },
] as const;
...
export function EmailTemplatesTab({ organization }: EmailTemplatesTabProps) {
  ...
  const [activeTab, setActiveTab] = useState<"confirmation" | "reminder">("confirmation");
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success(`${activeTab === "confirmation" ? "Confirmation" : "Reminder"} email template saved successfully`);
    } catch (error) {
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Customize the email notifications sent to customers for appointment confirmations and reminders.
            Use variables like {"{{customerName}}"} to insert dynamic content.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as "confirmation" | "reminder")}>
            ...
            <TabsContent value={activeTab} className="space-y-4 mt-4">
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                <div className="lg:col-span-2 space-y-4">
                  ...
                  <div className="flex gap-2">
                    <Button onClick={handleSave} disabled={updateOrganizationMutation.isPending}>
                      ...
                      {updateOrganizationMutation.isPending ? "Saving..." : "Save Template"}
                    </Button>
                ...
                <div className="space-y-4">
                  ...
                  <Card>
                    <CardHeader>
                      <CardTitle className="text-sm">Conditional Blocks</CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-2 text-sm text-muted-foreground">
                      <p>Use conditional blocks to show content only when a variable exists:</p>
                      <pre className="bg-muted p-2 rounded text-xs font-mono">
{`{{#if fee}}
  Fee: {{fee}}
{{/if}}`}
                      </pre>
                      <p className="text-xs">Supported: fee, organizationAddress, organizationPhone, organizationEmail</p>
                    </CardContent>
                  </Card>
                </div>
              </div>
