...
import { useSearchCustomers, useSearchAppointments } from "@/hooks";
import { useState, useEffect, useRef } from "react";
import { useNavigate } from "react-router-dom";
import { cn } from "@/lib/utils";
import { Customer, Appointment } from "@/core";
...
export function SiteHeader() {
  ...
  // Search hooks
  const { data: customers = [], isLoading: isLoadingCustomers } = useSearchCustomers(
    debouncedQuery,
    10
  );
  const { data: appointments = [], isLoading: isLoadingAppointments } = useSearchAppointments(
    debouncedQuery,
    10
  );
