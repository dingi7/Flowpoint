import { deleteServiceApiFn } from "@/app/api/services/delete-service";
...
export const apiDeleteService = onRequest(
  {
    invoker: "public",
  ...
  async (req: AuthenticatedRequest, res) => {
    ...
    try {
      ...
      loggerService.error("Error deleting service via API", error);
      res.status(500).json({
        error:
