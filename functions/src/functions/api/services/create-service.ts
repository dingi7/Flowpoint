import { createServiceApiFn } from "@/app/api/services/create-service";
...
export const apiCreateService = onRequest(
  ...
  async (req: AuthenticatedRequest, res) => {
    ...
    try {
      ...
      const serviceId = await createServiceApiFn(
        {
      ...
      loggerService.error("Error creating service via API", error);
      res.status(500).json({
        error:
