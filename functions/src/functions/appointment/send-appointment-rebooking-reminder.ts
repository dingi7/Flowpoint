...
import { sendAppointmentRebookingReminderEmailFn } from "@/app/notification/send-appointment-rebooking-reminder-email";
...
/**
 * HTTP endpoint to send appointment rebooking reminder emails
 * Called by Cloud Tasks at the scheduled time
 */
export const sendAppointmentRebookingReminder = onRequest(
  {
    invoker: "public",
  ...
  async (request, response) => {
    try {
      ...
      await sendAppointmentRebookingReminderEmailFn(
        payload,
        {
      ...
      response.status(200).send("Rebooking reminder email sent successfully");
    } catch (error) {
      loggerService.error("Error sending rebooking reminder email", error);
      response.status(500).send(
        `Error sending rebooking reminder: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  },
);
