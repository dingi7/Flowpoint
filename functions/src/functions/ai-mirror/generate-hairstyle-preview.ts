import { generateHairstylePreviewFn } from "@/app/ai-mirror/generate-hairstyle-preview";
...
export const generateHairstylePreview = onCall<Payload>(
  {
    invoker: "public",
  ...
  async (request) => {
    try {
      ...
      const genkitService = serviceHost.getGenkitService({ apiKey });
      const result = await generateHairstylePreviewFn(request.data, {
        genkitService,
        loggerService,
      });
