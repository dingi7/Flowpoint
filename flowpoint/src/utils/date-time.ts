...
export function convertWorkingHoursToLocal(
 ...
 */
export function getDateRangeForQuery(date: Date): {
  startOfDay: string;
  endOfDay: string;
} {
 ...
 */
export function normalizeDateToNoon(
  year: number,
  month: number,
  day: number,
): Date {
 ...
 */
export function normalizeApiKey<T extends Record<string, unknown>>(
  apiKey: T,
): T & { createdAt: Date | null } {
