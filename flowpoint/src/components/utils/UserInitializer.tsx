...
export function UserInitializer({ children }: { children: React.ReactNode }) {
  ...
  const { setUser, setLoading, setError, setInitialized, clearError } =
    useUserActions();
  const {
    setOrganizations,
  ...
  } = useGetOrganizationsByIds(organizationIds);
  ...
  useEffect(() => {
    ...
    if (organizationsError) {
      setOrgError(
        organizationsError.message || "Failed to load organization data",
      );
      return;
    }
